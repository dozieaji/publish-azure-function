...
resource "azurerm_function_app" "function_app" {
  ...
  site_config {
    linux_fx_version          = var.os == "linux" ? "PYTHON|3.7" : null
...
locals {
    publish_code_command = "az functionapp deployment source config-zip --resource-group ${azurerm_resource_group.resource_group.name} --name ${azurerm_function_app.function_app.name} --src ${var.archive_file.output_path}"
}
